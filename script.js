...
async function initializeApp() {
    if (!isJobsSection()) return; // Only run if we're on jobs section
...
function handleLogin(event) {
    ...
    initializeApp(); // Initialize jobs after login
}
...
// Add this function to check if we're on the jobs section
function isJobsSection() {
    return !document.getElementById('jobsSection').classList.contains('hidden');
} 